 ...
 * $Id: list.c,v 1.1.1.1 2000/06/23 05:55:53 edwards Exp $ 
 */
...
int add_list_strcmp(List *item1, List *item2)
{
	return strcmp(item1->name, item2->name);
}

int add_list_stricmp(List *item1, List *item2)
{
 ...
 */
void add_to_list_ext(List **list, List *add, int dble, int (*cmp_func)(List *, List *))
{
register List	*tmp;
	 List	*last;
...
{
	add_to_list_ext(list, add, 0, NULL);
}

void add_to_list_double(List **list, List *add)
{
	add_to_list_ext(list, add, 1, NULL);
}
 ...
 * is done 
 */
List	* find_in_list_ext(register List **list, char *name, int wild, int (*cmp_func)(List *, char *))
{
register List	*tmp;
	int	best_match,
		current_match;
...
List	* find_in_list(List **list, char *name, int wild)
{
 ...
 */
List	*remove_from_list_ext(List **list, char *name, int (*cmp_func)(List *, char *))
{
register	List	*tmp;
		List 	*last;
...
List	*remove_from_list(List **list, char *name)
{
	return remove_from_list_ext(list, name, NULL);
}

List	*remove_from_list_double(List **list, char *name)
{
List *tmp = NULL;
	if ((tmp = remove_from_list_ext(list, name, NULL)))
	{
		if (tmp->prev)
...
List	*removewild_from_list(List **list, char *name)
{
 ...
 * code 
 */
List	*list_lookup_ext(register List **list, char *name, int wild, int delete, int (*cmp_func)(List *, char *))
{
register List	*tmp;
...
List	*list_lookup(List **list, char *name, int wild, int delete)
{
	return list_lookup_ext(list, name, wild, delete, NULL);
}

List *break_from_list(List **list, List *tmp)
{
	if (tmp->prev)
		tmp->prev->next = tmp->next;
	else

 ...
 * $Id: if.c,v 1.1.1.1 2001/02/02 23:58:36 edwards Exp $
 */
...
BUILT_IN_COMMAND(ifcmd)
{
	...
	while (args && *args)
	{
		...
		if (!current_expr)
		{
			naperror("IF: Missing expression");
			return;
		}
		current_expr_val = parse_inline(current_expr, subargs, &flag);

		if (internal_debug & DEBUG_EXPANSIONS && !in_debug_yell)
			debugyell("%3d %s expression expands to: (%s)", debug_count, command, current_expr_val);
...
BUILT_IN_COMMAND(whilecmd)
{
	...
	{
		naperror("WHILE: missing boolean expression");
		return;
	}
	exp = LOCAL_COPY(ptr);
...
BUILT_IN_COMMAND(foreach)
{
	...
	{
		naperror("FOREACH: missing structure expression");
		return;
	}
	...
	{
		new_free(&struc);
		naperror("FOREACH: missing variable");
		return;
	}
	while (my_isspace(*args))
		args++;
	...
	{
		new_free(&struc);
		naperror("FOREACH: missing statement");
		return;
	}
...
BUILT_IN_COMMAND(fe)
{
	...
	if (!list)
	{
		naperror("%s: Missing List for /%s", command, command);
		return;
	}
	...
	if (!(args = strchr(args, '{')))		/* } */
	{
		naperror("%s: Missing commands", command);
		new_free(&templist);
		return;
	}
	if ((char *)var == (char *)args)
	{
		naperror("%s: You did not specify any variables", command);
		new_free(&templist);
		return;
	}
	args[-1] = '\0';
	...
	while ((var[ind++] = next_arg(vars, &vars)))
	{
		if (ind == 255)
		{
			naperror("%s: Too many variables", command);
			new_free(&templist);
			return;
		}
	}
...
BUILT_IN_COMMAND(forcmd)
{
	...
	{
		naperror("FOR: missing closing parenthesis");
		return;
	}
	...
	if (!iteration)
	{
		naperror("FOR: Only two components!");
		return;
	}
	do 
	{
	...
	{
		naperror("FOR: badly formed commands");
		return;
	}
...
BUILT_IN_COMMAND(switchcmd)
{
	...
	{
		naperror("SWITCH: String to be matched not found where expected");
		return;
	}
	...
	if (internal_debug & DEBUG_EXPANSIONS && !in_debug_yell)
		debugyell("%3d %s expression expands to: (%s)", debug_count, command, control);

	if (!(body = next_expr(&args, '{')))
		naperror("SWITCH: Execution body not found where expected");
	...
	while (body && *body)
	{
		...
		while (*body == '(')
		{
			...
			{
				naperror("SWITCH: Case label not found where expected");
				new_free(&control);
				return;
			}
			if (!strcmp(header, "default"))
			{
				if (def)
				{
					naperror("SWITCH: No more than one \"default\" case");
					new_free(&control);
					return;
				}
				found_def = 1;
			...
			if (internal_debug & DEBUG_EXPANSIONS && !in_debug_yell)
				debugyell("%3d %s expression expands to: (%s)", debug_count, command, header);
		...
		{
			naperror("SWITCH: case body not found where expected");
			break;
		}

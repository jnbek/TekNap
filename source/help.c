 ...
 * $Id: help.c,v 1.1.1.1 2000/10/16 01:09:37 edwards Exp $
 */
...
static	int	show_help (Window *window, char *name)
{
	...
	if (get_int_var(HELP_PAGER_VAR))
		rows = target_window->display_size;
...
static	void	help_topic (char *path, char *name)
{
	...
	if ((help_fp = uzfopen (&filename, path, 0, NULL)))
	{
...
static 	void 	help_pause_add_line (char *format, ...)
{
	...
	if ((help_paused_lines + 1) >= HELP_PAUSED_LINES_MAX)
		nappanic("help_pause_add_line: would overflow the buffer");
...
BUILT_IN_COMMAND(help)
{
	...
	if (!help_path || !*help_path || access(help_path, R_OK | X_OK))
	{
		...
		help_put_it(no_help, "*** /set help_path %s  (in most cases)", 
#ifdef WINNT
			"//c/TekNap/help"
#else
			"/usr/local/share/TekNap/help"
#endif
		);
		return;
	}
...
static	void	help_put_it	(const char *topic, const char *format, ...)
{
	...
	if (format)
	{
		...
		if (do_hook(HELPWIN_LIST, "%s %s", topic, putbuf))
		{

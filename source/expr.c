 ...
 *  $Id: expr.c,v 1.1.1.1 2001/01/14 10:56:45 edwards Exp $
 */
...
static char *canon_number (char *input)
{
	...
	if (get_int_var(FLOATING_POINT_VAR) == 1)
	{
 ...
 */ 
char	*next_unit (char *str, const char *args, int *arg_flag, int stage)
{
...
char	*expand_alias	(const char *string, const char *args, int *args_flag, char **more_text)
{
	...
	if (internal_debug & DEBUG_EXPANSIONS && !in_debug_yell && alias_debug)
		debugyell("%3d Expanded [%s] to [%s] (%s)", debug_count, string, buffer, args ? args : empty_string);
 ...
 */
char	*alias_special_char(char **buffer, char *ptr, const char *args, char *quote_em, int *args_flag)
{
	char	*tmp,
		*tmp2,
		pad_char = 0;

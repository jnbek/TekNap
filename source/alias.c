 ...
 * $Id: alias.c,v 1.1.1.1 2001/02/25 05:24:45 edwards Exp $
 */
...
BUILT_IN_COMMAND(aliascmd)
{
	...
	if ((p = strchr(name, LEFT_PAREN)))
	{
		arglistptr = p;
		*arglistptr++ = 0;
		*(args-1) = ' ';
		ptr = MatchingBracket(arglistptr, LEFT_PAREN, RIGHT_PAREN);
		if (!ptr)
			say("Unmatched lparen in %s %s", command, name);
		else
		{
			*ptr++ = 0;
			args = ptr;
...
extern u_32int_t       char_searches;
	...
	else
	{
		...
		{
			while (*arglistptr && my_isspace(*arglistptr))
				arglistptr++;
			arglist = parse_arglist(arglistptr);
		}
		/*
...
BUILT_IN_COMMAND(stubcmd)
{
	...
	{
		naperror("[%s] is an Unrecognized stub type", cmd);
		return;
	}
...
BUILT_IN_COMMAND(dumpcmd)
{
	...
	{
		if (!my_strnicmp(blah,"BIND",4) || !my_strnicmp(blah, "KEY", 3))
		{
			remove_bindings();
			init_keys();
			init_keys2();
...
void	add_var_alias	(char *name, char *stuff, int noisy)
{
	...
	if (*ptr && noisy)
  		naperror("ASSIGN names may not contain '%c' (You asked for [%s])", *ptr, name);
...
void	add_local_alias	(char *name, char *stuff, int noisy)
{
	...
	{
		if (noisy)
			naperror("LOCAL names may not contain '%c' (You asked for [%s])", *ptr, name);	
		new_free(&name);
		return;
	}
	/*
...
void	add_var_stub_alias  (char *name, char *stuff)
{
	...
	if (*ptr)
		...
		naperror("You may not stub the FUNCTION_RETURN variable.");
...
static void unstub_alias (Alias *item)
{
	...
	already_looking = 1;
	load("LOAD", copy, empty_string, NULL, 0);
	already_looking = 0;
}

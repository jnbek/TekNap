 ...
 * $Id: exec.c,v 1.1.1.1 2000/08/28 16:36:34 edwards Exp $
 */
 ...
 */
#if !defined(SYS_SIGLIST_DECLARED) && !defined(_SYS_SIGLIST_DECLARED)
...
static void 	handle_filedesc (Process *proc, int *fd, int hook_nonl, int hook_nl)
{
	...
	switch ((len = new_dgets(exec_buffer, *fd, 0, IO_BUFFER_SIZE)))	/* No buffering! */
	{
			...
			for (i = 0; i < process_list_size; i++)
			{
				...
				if (proc && proc->pid == pid)
				{
					...
					proc->retcode = WEXITSTATUS(status);
                                        ...
                                        WinSendMsg(last_input_screen->hwndFrame, WM_SETICON, (MPARAM)icon, 0);
#endif
					break;
				}
			}
...
static void 	cleanup_dead_processes (void)
{
	...
	for (i = 0; i < process_list_size; i++)
	{
		...
		{
			next = cmd->next;
			parse_line(NULL, cmd->name, exit_info, 0, 0, 1);
			new_free(&cmd->name);
			new_free((char **)&cmd);
		}
